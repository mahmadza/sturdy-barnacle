...
class ImageAlbumManager:
    ...
    class ImageAlbumManager:
        """Manages automatic grouping of images into albums using t-SNE + HDBSCAN."""
    ...
    def group_images_into_albums(self):
        """Clusters images using t-SNE + HDBSCAN and returns a structured list for Pandas."""
        # Reset album tables before new clustering
        ...
        for album_id, num_images in image_counts.items():
            album_data.append({"album_id": album_id, "album_name": f"Album {album_id}", "num_images": num_images})

        # Convert to DataFrame for visualization
        df = pd.DataFrame(album_data).sort_values(by="num_images", ascending=False)
    ...
    def reset_album_tables(self):
        ...
        try:
            session.execute(text("TRUNCATE TABLE image_album_mapping RESTART IDENTITY CASCADE;"))
            session.execute(text("TRUNCATE TABLE image_albums RESTART IDENTITY CASCADE;"))
            session.commit()
            print("Tables cleared successfully! Ready for a new clustering experiment.")
        finally:
            session.close()
